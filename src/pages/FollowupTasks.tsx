...
import { Phone, CheckCircle, Plus, Trash2, X, Save } from 'lucide-react';
...
export default function FollowupTasks() {
  ...
  const handleDone = async (id: number) => { await fetch(`http://localhost:3001/api/followups/${id}/done`, { method: 'PUT' }); refetch(); };
  ...
  const handleDelete = async (id: number) => {
    ...
    await fetch(`http://localhost:3001/api/followups/${id}`, { method: 'DELETE' });
    refetch();
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2">
        {[{ k: 'all', l: 'All' }, { k: 'pending', l: 'Pending' }, { k: 'done', l: 'Done' }].map(t =>
        ...
        <Card key={t.id} className={t.status === 'done' ? 'opacity-60' : ''}><CardContent className="p-3">
          <div className="flex justify-between items-start">
            <div className="flex gap-2 items-center">
              ...
              <span className={'text-xs px-1.5 py-0.5 rounded ' + (typeColors[t.task_type] || 'bg-gray-100')}>
                {TYPE_OPTIONS.find(o => o.value === t.task_type)?.label || t.task_type}
              </span>
            ...
            <div className="flex items-center gap-1">
              ...
              <button onClick={() => handleDelete(t.id)} className="p-1 text-red-600 hover:bg-red-50 rounded"><Trash2 size={14} /></button>
            </div>
          </div>
      ...
      {showForm && <div className="fixed inset-0 bg-black/40 z-50 flex items-end md:items-center justify-center" onClick={() => setShowForm(false)}>
        <div className="bg-white rounded-t-xl md:rounded-xl w-full md:w-[500px] max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
          <div className="sticky top-0 bg-white border-b p-4 flex justify-between items-center">
            <h3 className="font-bold">New Task</h3><button onClick={() => setShowForm(false)} className="text-gray-400"><X size={18} /></button>
          </div>
          <form onSubmit={handleSubmit} className="p-4 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><label className="text-xs text-gray-500">Type</label>
                <select value={form.task_type} onChange={e => setForm({ ...form, task_type: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm">{TYPE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
