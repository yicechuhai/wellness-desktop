...
export default function ServiceRecords() {
  ...
  const handleDelete = async (id: number) => {
    ...
    await fetch(`/wellness/api/services/${id}`, { method: 'DELETE' });
    refetch();
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2">
        ...
        {can('export') && <button onClick={() => { const t = localStorage.getItem('wellness_token'); fetch('/wellness/api/export/service_record', { headers: { Authorization: `Bearer ${t}` } }).then(r => r.blob()).then(b => { const u = URL.createObjectURL(b); const a = document.createElement('a'); a.href = u; a.download = `services_${new Date().toISOString().slice(0,10)}.xlsx`; a.click(); URL.revokeObjectURL(u); }); }} className="bg-green-600 text-white px-3 py-2 rounded-lg text-sm flex items-center gap-1 hover:bg-green-700" title="Export"><Download size={16} /></button>}
      </div>
