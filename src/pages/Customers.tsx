...
export default function Customers() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editing) { await fetch(`/wellness/api/customers/${editing.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('wellness_token')}` }, body: JSON.stringify(form) }); }
    else { await createMut.post(form); }
    setShowForm(false); resetForm(); refetch();
  };

  const handleDelete = async (id: number) => { if (!confirm('Delete?')) return; await fetch(`/wellness/api/customers/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${localStorage.getItem('wellness_token')}` } }); refetch(); setSelected(null); };

  const handleExport = () => {
    const token = localStorage.getItem('wellness_token');
    fetch('/wellness/api/export/customer', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.blob())
