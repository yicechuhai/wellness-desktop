...
export default function ServiceItems() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editing) { await fetch(`/wellness/api/items/${editing.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('wellness_token')}` }, body: JSON.stringify(body) }); }
    else { await fetch('/wellness/api/items', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('wellness_token')}` }, body: JSON.stringify(body) }); }
    setShowForm(false); resetForm(); refetch();
  };

  const handleDelete = async (id: number) => { if (!confirm('Delete?')) return; await fetch(`/wellness/api/items/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${localStorage.getItem('wellness_token')}` } }); refetch(); };
  const doExport = () => { const t = localStorage.getItem('wellness_token'); fetch('/wellness/api/export/service_item', { headers: { Authorization: `Bearer ${t}` } }).then(r => r.blob()).then(b => { const u = URL.createObjectURL(b); const a = document.createElement('a'); a.href = u; a.download = `items_${new Date().toISOString().slice(0,10)}.xlsx`; a.click(); URL.revokeObjectURL(u); }); };
